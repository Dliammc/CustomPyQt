...
class CLineEdit(QtWidgets.QWidget):
    def __init__(
        ...
        self._border_width = (
            ThemeManager.theme["CLineEdit"]["border_width"] if border_width is None else border_width
            )
        self._corner_radius = (
            ThemeManager.theme["CLineEdit"]["corner_radius"] if corner_radius is None else corner_radius
            )
        self._text_color = (
            ThemeManager.theme["CLineEdit"]["text_color"] if text_color is None else text_color
            )
        self._placeholder_text_color = (
            ThemeManager.theme["CLineEdit"]["placeholder_text_color"] if placeholder_text_color is None else placeholder_text_color
            )
        self._background_color = (
            ThemeManager.theme["CLineEdit"]["background_color"] if background_color is None else background_color
            )
        self._border_color = (
            ThemeManager.theme["CLineEdit"]["border_color"] if border_color is None else border_color
            )
        self._disabled_text_color= (
            ThemeManager.theme["CLineEdit"]["disabled_text_color"] if disabled_text_color is None else disabled_text_color
            )
        self._disabled_background_color = (
            ThemeManager.theme["CLineEdit"]["disabled_background_color"] if disabled_background_color is None else disabled_background_color
            )
    ...
    def icon(self, icon: Optional[str] = None):
        ...
        self._line_edit.setIcon(QtGui.QIcon(self._icon))

    @icon_size.setter
    def icon_size(self, icon_size: Tuple[int, int] = (16, 16)):
        self._icon_size = icon_size

        self._line_edit.setIconSize(QtCore.QSize(self._icon_size[0], self._icon_size[1]))
